...
INSTALL="sudo apt-get -qq install -y"
UPDATE="sudo apt-get -qq update"
...
sudo install -D -o root -g root -m 644 sublimehq-pub.gpg /usr/share/keyrings/sublimehq-pub.gpg
...
sudo install -D -o root -g root -m 644 google-chrome-pub.gpg /usr/share/keyrings/google-chrome-pub.gpg
# Add Google Chrome repository to apt sources list
echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/google-chrome-pub.gpg] http://dl.google.com/linux/chrome/deb/ stable main" | sudo tee /etc/apt/sources.list.d/google-chrome.list > /dev/null
# Delete the already installed GPG key
...
if [[ -f jetbrains-toolbox.sh ]]; then
    sudo ./jetbrains-toolbox.sh && jetbrains-toolbox
...
# - GitHub Desktop
echo "- GitHub Desktop"
./retrieve_latest_release.py --gh-user shiftkey --gh-repo desktop --pattern .deb
echo ""
# Documentation:
# - Obsidian
echo "- Obsidian"
