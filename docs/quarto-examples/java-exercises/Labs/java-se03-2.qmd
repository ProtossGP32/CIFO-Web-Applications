...
When updating a `Borrow`, a similar form as when creating one shall appear, but this time allowing the user to modify other fields such as `dueBorrowDate`, `returnedBorrowDate` and `borrowStatus`.
